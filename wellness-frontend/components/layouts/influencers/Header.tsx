...
const Header: React.FC<HeaderProps> = ({ isCollapsed }) => {
  ...
  const generateBreadcrumbs = (): Breadcrumb[] => {
    ...
    const breadcrumbs: Breadcrumb[] = [
      { name: 'Dashboard', href: '/influencers', icon: Home }
    ]
  ...
  const displayName = mounted
    ? currentUser?.firstName && currentUser?.lastName
      ? `${currentUser.firstName} ${currentUser.lastName}`
      : currentUser?.firstName || 'Influencer'
    : 'Influencer'

  const displayEmail = mounted ? (currentUser?.email || 'influencer@example.com') : 'influencer@example.com'

  const initials = mounted
    ? currentUser?.firstName && currentUser?.lastName
      ? `${currentUser.firstName.charAt(0)}${currentUser.lastName.charAt(0)}`.toUpperCase()
      : currentUser?.firstName?.charAt(0)?.toUpperCase() || 'I'
    : 'I'
        ...
        <div className="flex items-center gap-1 sm:gap-2">
          ...
          <div className="relative hidden lg:block">
            ...
            <input
              ...
              className="pl-9 pr-4 py-2 w-56 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-700 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 focus:bg-white transition-all"
            />
          </div>
          ...
          {/* Notifications */}
          <button className="relative p-2 text-slate-500 hover:text-purple-600 hover:bg-purple-50 rounded-xl transition-colors">
          ...
          {/* Settings */}
          <Link
            href="/influencers/settings"
            className="hidden sm:flex p-2 text-slate-500 hover:text-purple-600 hover:bg-purple-50 rounded-xl transition-colors"
          >
          ...
          <div className="flex items-center gap-2.5">
            <Avatar className="w-8 h-8 border-2 border-white shadow-sm ring-2 ring-slate-100">
              ...
              <AvatarFallback className="text-xs font-semibold bg-purple-50 text-purple-700">
                {initials}
              </AvatarFallback>
            </Avatar>
